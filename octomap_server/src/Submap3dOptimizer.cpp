  ...
  m_initConfig(true)
{
  ...
  m_tfPosePointCloudSub->registerCallback(boost::bind(&Submap3dOptimizer::subNodeMapCallback, this, _1));


  // service
  m_octomapBinaryService = m_nh.advertiseService("octomap_binary", &Submap3dOptimizer::octomapBinarySrv, this);
  m_octomapFullService = m_nh.advertiseService("octomap_full", &Submap3dOptimizer::octomapFullSrv, this);
  m_clearBBXService = m_nh_private.advertiseService("clear_bbx", &Submap3dOptimizer::clearBBXSrv, this);
  m_resetService = m_nh_private.advertiseService("reset", &Submap3dOptimizer::resetSrv, this);
...
void Submap3dOptimizer::insertPC(const Pose& sensorOrigin, const PCLPointCloud& nonground){
  ros::WallTime startTime = ros::WallTime::now();
...
// processing one pointcloud input
void Submap3dOptimizer::insertScan(const tf::Point& sensorOriginTf, const PCLPointCloud& nonground){
...
void Submap3dOptimizer::publishOCTmap3d(const ros::Time& rostime){
  ros::WallTime startTime = ros::WallTime::now();
  size_t octomapSize = m_octree->size();
  if (octomapSize <= 1){
    ROS_WARN("Nothing to publish, octree is empty");
...
void Submap3dOptimizer::publishBinaryOctoMap(const ros::Time& rostime) const{
...
void Submap3dOptimizer::publishFullOctoMap(const ros::Time& rostime) const{
...
bool Submap3dOptimizer::octomapBinarySrv(OctomapSrv::Request  &req,
...
bool Submap3dOptimizer::octomapFullSrv(OctomapSrv::Request  &req,
                                    OctomapSrv::Response &res)
{
...
bool Submap3dOptimizer::clearBBXSrv(BBXSrv::Request& req, BBXSrv::Response& resp){
...
bool Submap3dOptimizer::resetSrv(std_srvs::Empty::Request& req, std_srvs::Empty::Response& resp) {
...
void Submap3dOptimizer::handlePreNodeTraversal(const ros::Time& rostime){
  if (m_publish2DMap){
...
void Submap3dOptimizer::handlePostNodeTraversal(const ros::Time& rostime){

  if (m_publish2DMap)
    m_projectedMapPub.publish(m_gridmap);
}

void Submap3dOptimizer::handleOccupiedNode(const OcTreeT::iterator& it){
...
void Submap3dOptimizer::handleFreeNode(const OcTreeT::iterator& it){
...
void Submap3dOptimizer::handleOccupiedNodeInBBX(const OcTreeT::iterator& it){
...
void Submap3dOptimizer::handleFreeNodeInBBX(const OcTreeT::iterator& it){
...
void Submap3dOptimizer::update2DMap(const OcTreeT::iterator& it, bool occupied){
...
bool Submap3dOptimizer::isSpeckleNode(const OcTreeKey&nKey) const {
  OcTreeKey key;
...
void Submap3dOptimizer::adjustMapData(nav_msgs::OccupancyGrid& map, const nav_msgs::MapMetaData& oldMapInfo) const{
...
std_msgs::ColorRGBA Submap3dOptimizer::heightMapColor(double h) {
